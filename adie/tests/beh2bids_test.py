...
def test_beh2bids():
    ...
    result = runner.invoke(beh2bids.main,
        ["-s", "CONADIE999", "-t", "nosuchtask", "-p", "ecg", f"{str(bids_dir)}"])
    assert result.exit_code == 0
...
def test_find_file():
    exit = beh2bids.find_file("ADIE999_F", "*.csv",
          bids_dir / "sourcedata" / "faketask")
    assert exit == 1
    exit = beh2bids.find_file("ADIE999_F", "*.csv",
          bids_dir / "sourcedata" / "mytask")
    assert exit == 1

    ses = beh2bids.check_session("ADIE999_F",
      source_task=bids_dir / "sourcedata" / "mytask")
    for s in ses:
        exit = beh2bids.find_file(s, "*.csv",
            bids_dir / "sourcedata" / "mytask")
        assert exit == bids_dir / f"sourcedata/mytask/level1/anotherlevel/{s}/data.csv"
    ...
    exit = beh2bids.find_file(ses[0], "**/*.csv",
        bids_dir / "sourcedata" / "mytask")
    assert exit == bids_dir / f"sourcedata/mytask/edge_case/level1/{ses[0]}/somedir/data.csv"

def test_check_session():
    exit = beh2bids.check_session("ADIE999_F",
      source_task=bids_dir / "sourcedata" / "faketask")
    assert exit == 1
