...
def extract_stimtype(data: pd.DataFrame,
...
def compile_probes(data):
    ...
    columns = ["StimIndex", 'nBack', 'fixStart', 'stimStart',
        'mwType', 'keyResp', 'respRT']
...
def compile_ses(data):
    ...
    col = data["mwType"].values.tolist() + \
        subj_info.columns.tolist()
    val = data["keyResp"].values.tolist() + \
        subj_info.loc[0, :].values.tolist()
...
def compile_performance(data):
    ...
    cond = pd.pivot_table(task_perf, index=["participant_id", "ses", "nBack"],
        values=["acc", "respRT"])
    overall = pd.pivot_table(task_perf, index=["participant_id", "ses"],
        values=["acc", "respRT"])
