...
def gen_ses_label(sub):
    ...
    drs = glob.glob(
        "/research/cisc2/projects/critchley_adie/*/CISC{}/*".format(sub)
    )
    # save names of session level directories
    return [os.path.split(d)[1] for d in drs if "DS_Store" not in d]


def run_heudiconv_func(
    sub, ses, out, getmetadata
    ...
    data = (
        "/research/cisc2/projects/critchley_adie/*/CISC{subject}/{session}/*/*"
    )
...
if __name__ == "__main__":
    ...
    if getmetadata == "n":
        subs = glob.glob(
            "/research/cisc2/projects/critchley_adie/*_dicoms/CISC17875"
        )
